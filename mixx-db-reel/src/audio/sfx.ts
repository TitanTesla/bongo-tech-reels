...
export type SfxName =
  ...
  | 'lock' // match found (two-note success)
  | 'timer' // stopwatch tick roll
  | 'thud' // big text landing
  | 'draw' // lines drawing in (noise sweep)
  | 'pops' // nodes popping in (rising blips)
  | 'blip' // single pitched blip (param = frequency)
  | 'ripple' // Dijkstra exploration wave (param = wave index)
  | 'note' // one path-lock note (param = frequency)
  | 'swish' // cards sorting
  | 'cash' // fare counter tick roll
  | 'ding' // arrival / fare settled
  | 'count' // 10,000 -> 12 rolldown sweep
  | 'engine' // boda putt-putt during the ride
  ...
  play(name: SfxName, param?: number) {
    ...
    switch (name) {
      ...
      case 'blip':
        ...
        break
      case 'ripple':
        this.tone({ f: 520 + (param ?? 0) * 70, f1: 300, dur: 0.45, g: 0.06 })
        break
      case 'note':
      ...
      case 'cash':
        for (let i = 0; i < 10; i++) {
          this.tone({ f: 1800, dur: 0.02, g: 0.045, at: i * 0.13, type: 'square' })
        }
        break
      case 'ding':
        ...
        break
      case 'count':
        this.tone({ f: 1600, f1: 220, dur: 1.5, g: 0.05, type: 'triangle' })
        break
      case 'engine':
        // boda putt-putt: sawtooth bursts for the 3.6s ride
        for (let i = 0; i < 24; i++) {
          this.tone({ f: 72 + Math.sin(i * 1.7) * 7, dur: 0.07, g: 0.09, at: i * 0.15, type: 'sawtooth' })
        }
        break
    }
  }
